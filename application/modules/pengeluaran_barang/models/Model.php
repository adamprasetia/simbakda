...
{
	private $tbl_name = 'pengeluaran_barang';
	private $tbl_key 	= 'id';

	public function query()
	{
		$data[] = $this->db->select('a.*,count(b.id) as jumlah,sum(b.jumlah*b.harga) as total,c.name as tahun_anggaran_name');
		...
		if($date_from <> '' && $date_to <> ''){
			$data[] = $this->db->where('a.tanggal_keluar >=',format_ymd($date_from));
			$data[] = $this->db->where('a.tanggal_keluar <=',format_ymd($date_to));
		}		
		...
		$data[] = $this->db->order_by($this->general->get_order_column('a.tanggal_keluar'),$this->general->get_order_type('desc'));
