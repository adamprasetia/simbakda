...
class Bidang extends MY_Controller 
{
	...
	{
		parent::__construct();
		$this->data['index'] = 'bidang';
		$this->data['title'] = 'Bidang';
		$this->data['code'] = '01';
	...
	public function index()
	{
		...
		$total 	= $this->inherit_model->count_all($this->data['index'],'',$this->data['code']);
		...
		$result = $this->inherit_model->get($this->data['index'],'',$this->data['code'])->result();
		$i=1+$offset;
		foreach($result as $r){
			$this->table->add_row(
	...
	public function add()
	{
		...
		if($this->form_validation->run()===false){
			...
			$this->model->add($this->data['index'],$data);
	...
	public function edit($id)
	{
		$this->_set_rules();
		if($this->form_validation->run()===false){
			$this->data['row'] = $this->model->get_from_field($this->data['index'],'id',$id)->row();
			...
			$this->model->edit($this->data['index'],$id,$data);
	...
	{
		if($id<>''){
			$this->model->delete($this->data['index'],$id);
		}
		$check = $this->input->post('check');
		if($check<>''){
			foreach($check as $c){
				$this->model->delete($this->data['index'],$c);
			}
		}
