...
class Bidang_unit extends MY_Controller 
{
	private $data = array();
	...
	{
		parent::__construct();
		$this->data['index'] = 'bidang/bidang_unit';
		$this->data['title'] = 'Unit Bidang';
		$this->data['index_parent'] = 'bidang';
		$this->data['title_parent'] = 'Bidang';
		$this->data['code'] = '02';
	...
	public function index()
	{
		...
		$total 	= $this->inherit_model->count_all($this->data['index_parent'],$this->data['index_parent'],$this->data['code']);
		...
		$result = $this->inherit_model->get($this->data['index_parent'],$this->data['index_parent'],$this->data['code'])->result();
		$i=1+$offset;
		foreach($result as $r){
			$this->table->add_row(
	...
	public function add()
	{
		...
		if($this->form_validation->run()===false){
			...
			$this->model->add($this->data['index_parent'],$data);
	...
	public function edit($id)
	{
		$this->_set_rules();
		if($this->form_validation->run()===false){
			$this->data['row'] = $this->model->get_from_field($this->data['index_parent'],'id',$id)->row();
			...
			$this->model->edit($this->data['index_parent'],$id,$data);
	...
	{
		if($id<>''){
			$this->model->delete($this->data['index_parent'],$id);
		}
		$check = $this->input->post('check');
		if($check<>''){
			foreach($check as $c){
				$this->model->delete($this->data['index_parent'],$c);
			}
		}
